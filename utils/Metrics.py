...
def ranklist_by_sort(pos_items, all_items, score):
    # pos_score = np.array(pos_score)
    # neg_score = np.array(neg_score)
    # score = np.hstack((pos_score, neg_score))
    # all_items = np.hstack((pos_items, neg_items))
    sorted_index = np.argsort(score)
    sorted_socre = score[sorted_index]
    ...
    for i in sorted_items:
        if i in pos_items:
            ...
            r.append(0)
    a = auc(ground_truth=r, prediction=-sorted_socre)
    return r, a


def ranklist_by_sort_classfify(logits, labels):
    sorted_index = np.argsort(logits)
    r = []
    for i, index in enumerate(sorted_index):
        r.append(labels[i][index])
    return np.array(r)
